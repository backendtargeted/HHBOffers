...
async processCsvFile(filePath: string, jobId: string, headerMapping?: HeaderMapping, defaultOfferDate?: Date): Promise<ProcessingStats> {
    ...
    try {
      ...
      for (const data of transformedData) {
        try {
          ...
          logger.info('[OfferHistory] Checking data:', {
          ...
          if (data.offerAmount !== undefined && data.offerDate) {
            logger.info(`[FileProcessorService] Creating offer for property ${property.id}:`, {
              propertyAddress: property.property_address,
              offerAmount: data.offerAmount,
              offerDate: data.offerDate
            });
            ...
            logger.info(`[FileProcessorService] Skipping offer creation for property ${property.id} - missing data:`, {
  ...
  private transformRowToPropertyData(row: any, headerMapping?: HeaderMapping, defaultOfferDate?: Date): TransformedRowData {
    ...
    logger.info('[FileProcessorService] Raw CSV row:', row);
    ...
    logger.info('[FileProcessorService] Using header mapping:', mapping);
    ...
    const getValue = (fieldKey: keyof HeaderMapping, fallbacks: string[]): any => {
      ...
      if (mappedHeader && row[mappedHeader] !== undefined) {
        logger.info(`[FileProcessorService] Found value for ${fieldKey} using mapped header ${mappedHeader}:`, row[mappedHeader]);
        return row[mappedHeader];
      }
      for (const fallback of fallbacks) {
        if (row[fallback] !== undefined) {
          logger.info(`[FileProcessorService] Found value for ${fieldKey} using fallback ${fallback}:`, row[fallback]);
          return row[fallback];
        }
      }
      logger.info(`[FileProcessorService] No value found for ${fieldKey}`);
      return null;
    };
    ...
    if (rawOfferAmount !== null && rawOfferAmount !== '') {
      ...
      logger.info(`[FileProcessorService] Raw offer amount: "${rawOfferAmount}", processed to: "${offerAmount}"`);
    }

    let offerDate = getValue('offerDate', ['offer_date', 'date', 'Offer Date', 'OFFER DATE']);
    if (offerDate) {
      logger.info(`[FileProcessorService] Found offer date: "${offerDate}"`);
    }
    ...
    if (!offerDate && defaultOfferDate) {
      ...
      logger.info(`[FileProcessorService] Using default offer date: "${offerDate}"`);
    }
    ...
    logger.info(`[FileProcessorService] Transformed row data:`, {
