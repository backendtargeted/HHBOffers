    ...
    try {
      ...
      setImmediate(async () => {
        try {
          ...
          if (fileType === 'csv') {
            await fileProcessorService.processCsvFile(filePath, jobId, headerMapping, defaultOfferDate);
          } else if (fileType === 'xlsx') {
            await fileProcessorService.processXlsxFile(filePath, jobId, headerMapping, defaultOfferDate);
          }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      logger.error('Error in uploadFile:', errorMessage);
